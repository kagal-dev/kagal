...
- **`@kagal/proto`** — Generated protobuf-es types for
  the wire protocol and registry. Wire messages in
  `agent.proto`, `task.proto`, `nonce.proto`,
  `envelope.proto`. Registry types in `registry.proto`:
  KV record shapes (`AgentRecord`, `NameRecord`,
  `CertRecord`, `RevokedRecord`), enums (`KagalRole`,
  `AgentStatus`), and `AuthResult`. Published from the
  proto schema at [`proto/kagal/v1/`][proto-v1].
...
|-----|-------|
| `name:<name>` | `{ agentId }` |
| `agent:<agentID>` | `{ ready, quarantined, name, lastSeen, groups }` |
| `cert:<fingerprint>` | `{ agentId, registeredAt }` |
...
- **operator** — a human or tool managing the fleet.
  ...
  and forwards requests to the gateway with the
  resolved identity.
...
| **not_ready** | Known agent, WS not established or nonce not verified | WS connect allowed; other agent routes denied (503) |
| **quarantined** | Ready, but quarantined | Control WS, `cert_renew` task; operators can interact; no tunnels, no custom tasks, not listed as operational |
| **ready** | WS up, nonce verified, not quarantined | Full permissions for role |

The WS endpoint is the bootstrap route — the only
route where `not_ready` is an acceptable state.
...
7. Combine role + state flags (`ready`,
...
entry during registration (`ready: false`,
...
- **First connect (no nonce state)** →
  ...
  `ready: true, quarantined: true`
- **WS connect + nonce valid** →
  `ready: true` (quarantine preserved from
  DO's SQLite state)
- **WS connect + nonce mismatch** →
  `ready: true, quarantined: true`
- **WS disconnect** → `ready: false`
...
sequenceDiagram
  ...
  alt Has CA-signed certificate
    ...
    KV-->>S: { ready, quarantined }
    Note over S: not_ready accepted for WS
...
The consumer's DO Worker env extends `KagalEnv` (which
includes `KAGAL_REGISTRY: KVNamespace` via
`KagalRegistryEnv`) with their own bindings. The
consumer's frontend Worker
...
| `health` (agent) | Agent DO | internal | Per-agent health check |
| `ws` | Agent DO | agent (not_ready ok) | Control WebSocket, hibernation |
| `tunnel` | Agent DO | agent/operator (ready) | On-demand port forwarding |
...
| `health` (supervisor) | Supervisor DO | internal | Supervisor health check |
...
- [`envelope.proto`][proto-envelope] — `ServerMessage`
  ...
  consumer messages.
- [`registry.proto`][proto-registry] — KV record
  shapes (`AgentRecord`, `NameRecord`, `CertRecord`,
  `RevokedRecord`, `RegistryRecord`), enums
  (`KagalRole`, `AgentStatus`), and `AuthResult`.
...
- **KV state publication**: On connect, disconnect,
  ...
  (ready, quarantined, last_seen). This is the
...
|-------------|-------|
| `name:<name>` | `{"agentId":"..."}` |
| `agent:<agentID>` | `{"ready":true,"quarantined":false,"name":"...","lastSeen":"...","groups":["..."]}` |
| `cert:<sha256>` | `{"agentId":"...","registeredAt":"..."}` |
| `revoked:<sha256>` | `{"revokedAt":"...","reason":"...","certNotAfter":"..."}` |
...
[proto-registry]: proto/kagal/v1/registry.proto
[api]: docs/api.md
